...
{
	public class MovementJoystick : MonoBehaviour, ISimpleInputDraggable
	{
...
#pragma warning disable 0649
        ...
        [SerializeField] private Image _fakeJoystick; //!!!
        [SerializeField] private Image _fakeThumb; //!!!
...
#pragma warning restore 0649
        ...
        private float _fakeOpacity = 0f; //!!!
        [SerializeField, Range(0, 10f)] private float _opacityMultiplier = 4f;
		...
		private void OnUpdate()
		{
			...
			if( joystickHeld ) 
            {
				opacity = Mathf.Min( 1f, opacity + Time.unscaledDeltaTime * _opacityMultiplier );
				_fakeOpacity = Mathf.Min( 0f, _fakeOpacity + Time.unscaledDeltaTime * _opacityMultiplier ); //!!!
            }
			else 
            {
				opacity = Mathf.Max( 0f, opacity - Time.unscaledDeltaTime * _opacityMultiplier );
				_fakeOpacity = Mathf.Max( 1f, _fakeOpacity - Time.unscaledDeltaTime * _opacityMultiplier ); //!!!
            }
            ...
            //!!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            Color fakeColor = _fakeThumb.color;
			fakeColor.a = _fakeOpacity;
			_fakeThumb.color = fakeColor;

			if( background )
			{
				fakeColor = _fakeJoystick.color;
				fakeColor.a = _fakeOpacity;
				_fakeJoystick.color = fakeColor;
			}
            //!!!>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		}
	}
